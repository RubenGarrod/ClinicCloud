...
class PubmedSpider(scrapy.Spider):
    ...
    def parse_articles(self, response):
        ...
        try:
            ...
            if response.headers.get('content-type', b'').decode().startswith('text/xml') or response.text.strip().startswith('<?xml'):
                articles = response.xpath('//PubmedArticle')
            else:
                self.logger.error(f"Respuesta no es XML válido. Content-Type: {response.headers.get('content-type')}")
                return
        except Exception as e:
            self.logger.error(f"Error al parsear respuesta XML: {e}")
            return
        ...
        for article in articles:
            try:
                ...
                # ============================================================
                # NUEVOS CAMPOS: MeSH Terms y Metadata Adicional
                # ============================================================

                # Extraer MeSH Terms (Medical Subject Headings)
                # Estos son términos de categorización oficial de NLM
                mesh_terms = []
                mesh_headings = article.xpath('.//MeshHeading')
                for mesh in mesh_headings:
                    # DescriptorName es el término principal
                    descriptor = mesh.xpath('./DescriptorName/text()').get('')
                    if descriptor:
                        mesh_terms.append(descriptor)

                        # También extraer Qualifiers (subterms) si existen
                        qualifiers = mesh.xpath('./QualifierName/text()').getall()
                        for qualifier in qualifiers:
                            mesh_terms.append(f"{descriptor}/{qualifier}")

                # Extraer información del Journal
                journal = article.xpath('.//Journal/Title/text()').get('')
                if not journal:
                    # Fallback a abreviación ISO
                    journal = article.xpath('.//Journal/ISOAbbreviation/text()').get('')

                # Extraer DOI (Digital Object Identifier)
                doi = article.xpath('.//ArticleId[@IdType="doi"]/text()').get('')

                # Extraer tipos de publicación
                # Ejemplos: "Journal Article", "Clinical Trial", "Randomized Controlled Trial"
                publication_types = article.xpath('.//PublicationType/text()').getall()

                # Extraer idioma del artículo
                language = article.xpath('.//Language/text()').get('eng')
