# -*- coding: utf-8 -*-
"""
Modulo para la categorización de documentos médicos.
Este módulo contiene la clase `MedicalCategorizer` que utiliza un sistema de reglas
para categorizar documentos médicos basándose en palabras clave y términos específicos de cada categoría.
"""
import logging
...
# categorías por palabras clave que las definan
CATEGORIAS_MEDICAS = {
...
# terminos no especificos que pueden aparecer en cualquier categoria
...
class MedicalCategorizer:
    ...
    def categorizar_texto(self, titulo: str, abstract: str) -> List[Tuple[str, float]]:
        ...
        # Concatenar título y resumen para la categorización
        texto_completo = f"{titulo} {titulo} {abstract}"
        texto_completo = texto_completo.lower()
        
        # Solo usamos el método basado en reglas
    ...
    def _categorizar_con_reglas_mejorado(self, texto: str) -> List[Tuple[str, float]]:
        ...
        # penalizamos los términos generales para evitar que se lleven la puntuación
        # y se clasifique siempre como Medicina General
        total_generales = 0
        ...
        # es probable clasificarlo mal por lo tanto es mejor asiganrlo a medicina general
        if total_generales > 10:
            ...
            if all(puntuacion < 2.0 for puntuacion in puntuaciones.values()):
                puntuaciones["Medicina General"] += 5.0  #le damos más peso a Medicina General para evitar una mala elección 
        
        #titulo_peso = 3.0  # Factor de peso para que el titulo tenga mas peso que le cuerpo
        ...
        # se ordena por puntuación y se retornan las 4 mejores
...
def obtener_mejor_categoria(titulo: str, abstract: str) -> str:
    ...
    try:
        ...
        if categorias:  
            # Si se encuentran categorias, seleccionamos la mejor
            # es decir la primera en la matriz
            mejor_categoria = categorias[0][0]
            logger.debug(f"Mejor categoría seleccionada: {mejor_categoria}")
            return mejor_categoria
        else:
            # Si no se encuentra categoria: Medicina General
